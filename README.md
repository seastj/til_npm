...
- html 이 입력이 되는것은 `크로스 사이트 스크립트 공격` 가능성 있음
- 줄여서 `XSS` 위험이 존재함.
...
const AddForm = () => {
  ...
  return (
    <div>
      ...
      <div>
        <p>입력중인 내용:{data}</p>
        {/* 아래처럼 출력해봄 : 리액트에서 위험한 내용이고, html 형태 설정함*/}
        <p dangerouslySetInnerHTML={{ __html: data }}></p>
        {/* 최소한의 방지책 */}
        <p dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize }}></p>
      </div>
    </div>
  );
};
