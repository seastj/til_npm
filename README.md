...
- 절대로 캘린더를 직접 코딩하지 않는다.
- react-calendar 가 가장 수월함. (기능적음)
  - https://projects.wojtekmaj.pl/react-calendar/
- full-calendar 가 많이 활용 됨. (기능많음, 복잡함)
...
<Calendar
  calendarType="gregory"
  formatShortWeekday={formatShortWeekday}
  tileClassName={tileClassName} // 특정날짜에 클래스 적용하기
/>
```
...
<Calendar
  ...
  formatShortWeekday={formatShortWeekday}
  tileClassName={tileClassName} // 특정날짜에 클래스 적용하기
  tileContent={tileContent} // 타일의 내용 보여주기
/>
```

```jsx
// 3. 외부에서 받아온 자료 출력하기
const [allData, setAllData] = useState([]); // 원래는 가장 위가 좋은자리
const tileContent = ({ date }) => {
  const checkDay = moment(date).format("YYYY-MM-DD");
  // console.log(checkDay);
  // api 데이터와 체크날짜를 비교해서 결과를 찾아낸다.
...
useEffect(() => {
  // 백엔드 자료 받아서 처리하기 가장 좋은 자리 (return이 보이는곳)
  setAllData(todoApi);
}, []);
```
...
<Calendar
  ...
  tileClassName={tileClassName} // 특정날짜에 클래스 적용하기
  tileContent={tileContent} // 타일의 내용 보여주기
  formatDay={formatDay} // 기본 출력날짜 변경하기
/>
```
...
```jsx
// ?.  옵셔널 체인을 꼭 챙겨주자
<div>선택한 날짜정보 : {clickInfo?.title}</div>
```
...
<Calendar
  ...
  tileContent={tileContent} // 타일의 내용 보여주기
  formatDay={formatDay} // 기본 출력날짜 변경하기
  onClickDay={onClickDay} // 캘린더 날짜 선택시 처리하기
/>
```
...
<Calendar
  ...
  tileClassName={tileClassNameShow} // 일정이 있는 날짜만 css 적용하기
  ...
/>
```
...
<Calendar
  ...
  selectRange={true}
  //   value={selectedRange}
  tileClassName={tileClassNameRange2} // 일정이 있는 날짜만 css 적용하기
/>
```
